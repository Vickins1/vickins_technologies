...
function AnimatedCounter({
  ...
  return (
    <motion.div
      ...
      className="text-center p-4 bg-[var(--navbar-bg)]/50 rounded-lg"
    >
      <span className="text-4xl font-bold text-[var(--button-bg)]">{count}+</span>
...
export default function AboutSection() {
  return (
    <motion.section id="about" className="py-16 bg-[var(--card-bg)] rounded-lg shadow-lg relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-[var(--button-bg)]/10 to-transparent opacity-30" />
      <div className="container mx-auto px-4 relative z-10">
        <h2 className="text-4xl font-bold text-center mb-12">Empowering Your Future with Innovative Technology</h2>
        <div className="flex flex-col md:flex-row items-center gap-12">
          <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="md:w-1/2">
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="md:w-1/2">
            ...
            <a href="#contact" className="inline-block bg-[var(--button-bg)] text-[var(--navbar-text)] px-6 py-3 rounded-full hover:opacity-90 transition duration-300 shadow-md font-semibold">
              Partner with Us
            </a>
          </motion.div>
        </div>
